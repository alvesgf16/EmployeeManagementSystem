    ...
    public partial class ManageEmployeeView : ContentPage
    {
        ...
        private void AddNewEmployee_Clicked(object sender, EventArgs e)
        {
            if (ValidateEmployeeInformation())
            {
                ...
                // Create a new EmergencyContact object with the same EmployeeID
                EmergencyContact emergencyContact = new EmergencyContact(employeeId, ContactNameEntry.Text, ContactPhoneNumberEntry.Text);

                // Create a new Payment object with the same EmployeeID
                Payment payment = new Payment(employeeId, Convert.ToDecimal(SalaryEntry.Text), Convert.ToInt32(HoursWorkedEntry.Text), Convert.ToInt32(PerformanceEntry.Text));
